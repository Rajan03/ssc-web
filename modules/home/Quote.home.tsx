...
const Quote: React.FC<SanityQuoteHome> = ({ title, subtitle }) => {
  ...
  return (
    <div className="relative min-h-[30rem] w-full flex flex-col bg-primary-600 mt-[8rem] p-16 overflow-hidden">
      <Image src={'/bg-abstract.png'} alt={'bg'} fill />

      <div className="container mx-auto flex justify-between items-center flex-1 gap-8 flex-col md:flex-row">
        {/* TEXT */}
        ...
        <div className="flex flex-col justify-center items-center md:items-start gap-y-4">
          <AnimatedBtn link={'/contact'} text={'Let\'s Connect'} theme={"bg-black"} hoverTheme={"bg-neutral-800"} className={'text-3xl'} />
          </div>
      </div>
    </div>
  );
