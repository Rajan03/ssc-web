...
import {IManagerFormSchema, ITeam} from "@/types";
...
const ManagerForm = ({coaches, submitForm, loading}: {
    coaches: ITeam[], loading: boolean,
    submitForm: (_data: IManagerFormSchema) => Promise<{ ok: boolean }>
}) => {
    ...
    const defaultValues: IManagerFormSchema = {
        firstName: '',
        lastName: '',
        email: '',
    ...
    const {
        ...
        setValue,
        reset
    } = useForm<IManagerFormSchema>({
        defaultValues: defaultValues, resolver: zodResolver(schema)
    })

    const onSubmit = (data: IManagerFormSchema) => {
        data.type = 'manager';
